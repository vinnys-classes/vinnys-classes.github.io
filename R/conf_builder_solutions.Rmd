---
title: "confidence_builder_solutions"
output: pdf_document
date: "2025-11-16"
---
...
First is a 99% confidence interval and then a 80% confidence interval to give the two extremes. Note the change in the y-axis scale

```{r}
conf_builder(sample_size = 20,
             confidence_proportion = .99,
             num_of_intervals = 20,
             mean = 0,
             pop_variance = 1)

conf_builder(sample_size = 20,
             confidence_proportion = .80,
             num_of_intervals = 20,
             mean = 0,
             pop_variance = 1)
```
...
Easy enoguh to check, we change the mean from 0 to 50. Biggest difference is the intervals are now centered at 50 instead of 10. Note that the length is basically the same. 

The overall coverage rate does NOT change (just rerun the plot several times and it should average out to the same coverage rate)
...
So, we increase the population variance from 1 to 100. Notice the y-axis to get a sense of how long these intervals are
...
So, we lower the sample size to n = 5 (top graph) and then increase the sample size to 2000 (lower graph). Notably the intervals become much tighter as the sample size jumps upwards. 

The overall coverage rate does NOT change (just rerun the plot several times and it should average out to the same coverage rate)

```{r}
conf_builder(sample_size = 5,
             confidence_proportion = .95,
             num_of_intervals = 20,
             mean = 0,
             pop_variance = 1)

conf_builder(sample_size = 2000,
             confidence_proportion = .95,
             num_of_intervals = 20,
             mean = 0,
             pop_variance = 1)
